...
const generateSerialNo = (prefix) => {
  ...
  if (prefix.includes("_")) {
    // Replace underscores with year and month (no serial number)
    const prefixWithDate = prefix.replace(/_/g, year + month);
    return prefixWithDate; // Just prefix with date, no 3-digit serial
  } else {
    // No underscores: append year + month only (no serial number)
    return `${prefix}${year}${month}`; // Just prefix + year + month
  }
};

// Enhanced POST route with StorageLocation support
// In POST route - remove the unique serial check since we don't need it anymore
route.post("/addModels", async (req, res) => {
  ...
  try {
    ...
    const existingPart = await Model.findOne({
    ...
    // Generate SerialNo (just prefix + year + month)
    const serialNo = generateSerialNo(Prefix);
...
route.get("/getModel", async (req, res) => {
  try {
    ...
    if (storageLocation) {
      if (
        storageLocation.toLowerCase() === "unspecified" ||
    ...
    const parts = await Model.find(query)
      .sort(sortOptions)
      .skip(skip)
...
// In PUT route - simplified serial generation
route.put("/parts/:id", async (req, res) => {
  try {
    ...
    // Check if another part with the same PartNumber exists
...
route.get("/storage-locations", async (req, res) => {
  try {
    const locations = await Model.distinct("StorageLocation", {
